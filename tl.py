...
def save_training_graph(train_losses, save_path=results_dir):
    ...
    # Save the figure
    graph_path = os.path.join(save_path, "training_loss+transfer_learning.png")
...
def save_validation_graph(val_accuracies, save_path=results_dir):
    ...
    graph_path = os.path.join(save_path, "validation_accuracy_tranfer_learning.png")
    plt.savefig(graph_path)
    plt.close()
...
# Initialize pre-trained DenseNet121 model for transfer learning
...
# Only optimize parameters of the final layer since others are frozen
